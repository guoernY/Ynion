...
namespace Ynion {
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(YN_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(YN_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	{
		std::string filepath = FileDialogs::OpenFile("Ynion Scene (*.ynion)\0*.ynion\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".ynion")
		{
			YN_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("Ynion Scene (*.ynion)\0*.ynion\0");
		if (!filepath.empty())
		{
