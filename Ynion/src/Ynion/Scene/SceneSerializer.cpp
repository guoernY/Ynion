...
namespace Ynion {
	...
	static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		YN_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	{
		YN_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		{
			YN_CORE_ERROR("Failed to load .ynion file '{0}'\n     {1}", filepath, e.what());
			return false;
		}
		...
		YN_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				YN_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
